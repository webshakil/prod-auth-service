...
export const createAuthSession = async ({
  ...
  try {
    ...
    const sessionResult = await query(
      `INSERT INTO votteryy_auth_sessions 
       ...
       RETURNING id, session_id, is_first_time, step_number, 
                 user_details_collected, biometric_collected, security_questions_answered`,
      [
        userId,
        sessionId,
        isFirstTime,
    ...
    // ✅ Check if user has already completed steps in a previous session
    const existingFlagsResult = await query(
      `SELECT 
         COALESCE(MAX(user_details_collected::int), 0)::boolean as user_details_collected,
         COALESCE(MAX(biometric_collected::int), 0)::boolean as biometric_collected,
         COALESCE(MAX(security_questions_answered::int), 0)::boolean as security_questions_answered
       FROM votteryy_auth_sessions 
       WHERE user_id = $1 AND session_id != $2`,
      [userId, sessionId]
    );

    const existingFlags = existingFlagsResult.rows[0] || {};

    return {
      sessionId,
      isFirstTime,
      stepNumber: 1,
      // ✅ NEW: Return session flags (combine current + existing)
      sessionFlags: {
        userDetailsCollected: session.user_details_collected || existingFlags.user_details_collected || false,
        biometricCollected: session.biometric_collected || existingFlags.biometric_collected || false,
        securityQuestionsAnswered: session.security_questions_answered || existingFlags.security_questions_answered || false,
      },
    };
  } catch (error) {
  ...
  try {
    ...
    await query(
      ...
      [
        ...
        timezone || 'UTC',
        language || 'en_us',
        registrationIP || null,  // ✅ FIX: Use null instead of empty string for inet type
      ]
    );
...
export const verifySngineTokenAndCreateSession = async ({
  ...
  try {
    ...
    // ─────────────────────────────────────────────────────────────────────
    // STEP 5: DO NOT save to votteryy_user_details here!
    // Just pass the data to frontend for pre-filling the form.
    // The actual save happens at Step 4 (UserDetailsForm) - same as manual flow.
    // This ensures isFirstTime stays TRUE and user goes through all steps.
    // ─────────────────────────────────────────────────────────────────────
    // NOTE: Removed saveUserDetailsFromSngine() call - it was causing isFirstTime to be false
    
    // Just mark that we have Sngine data available for pre-fill
    ...
    const responseData = {
      ...
      nextStep: 2,
      authMethod: 'sngine_token',
      // ✅ NEW: Session flags to check what's already completed
