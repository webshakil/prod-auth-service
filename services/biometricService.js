  ...
  try {
    ...
    const result = await query(
      `INSERT INTO votteryy_user_biometrics 
       (user_id, session_id, device_id, biometric_type, biometric_data_hash, biometric_quality_score, is_primary)
       VALUES ($1, $2, $3, $4, $5, $6, $7)
       RETURNING id`,
      [
        userId,
        sessionId,
        biometricData.deviceId || null, // ✅ Store device_id
        biometricData.type,
        biometricHash,
...
export const getUserBiometrics = async (userId) => {
  try {
    const result = await query(
      `SELECT id, biometric_type, device_id, is_verified, is_primary, verification_count, 
    ...
    if (isVerified) {
      await query(
        `UPDATE votteryy_user_biometrics 
         SET is_verified = true, verification_count = verification_count + 1, last_used = CURRENT_TIMESTAMP 
         WHERE id = $1`,
        [biometricId]
      );
...
export const generateBackupCodes = async (userId) => {
  try {
    ...
    logger.info('Backup codes generated', { userId, count: backupCodes.length });
    return backupCodes;
  } catch (error) {
