...
export const completeAuthenticationController = async (req, res) => {
  try {
    ...
    if (session.authentication_status === 'completed') {
      logger.info('Session already completed - returning existing data', { sessionId });
      ...
      try {
        ...
        if (!userDetails) {
          logger.error('User profile not found for completed session', { userId });
        ...
        return sendSuccess(
          ...
          'Authentication already completed - session restored'
        );
      } catch (error) {
        logger.error('Error handling already completed session', { 
          error: error.message,
          sessionId,
        });
        return sendError(res, 'Failed to restore session', 500);
      }
    }
