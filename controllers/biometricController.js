...
export const collectBiometric = async (req, res) => {
  try {
    ...
    // Get session with biometric_collected flag
    const sessionResult = await query(
      `SELECT user_id, is_first_time, biometric_collected 
    ...
    // ✅ FIXED: Check if biometric already collected for THIS USER (not just this session)
    // Allow collection if user hasn't collected biometric before
    const existingBiometricResult = await query(
      `SELECT id FROM votteryy_user_biometrics 
       WHERE user_id = $1 AND is_primary = true 
       LIMIT 1`,
      [userId]
    );

    if (existingBiometricResult.rows.length > 0) {
      // User already has biometric registered
      logger.info('Biometric already registered for user', { userId });
      
      // Just update the session flag and return success
      await query(
        `UPDATE votteryy_auth_sessions 
         SET biometric_collected = true 
         WHERE session_id = $1`,
        [sessionId]
      );
    ...
    // Update session - Mark biometric as collected
    ...
    // ✅ Also update ALL previous sessions for this user to mark biometric collected
    await query(
      `UPDATE votteryy_auth_sessions 
       SET biometric_collected = true 
       WHERE user_id = $1`,
...
export const verifyBiometricController = async (req, res) => {
  try {
    ...
    // ✅ Original fingerprint verification code
    // Get session
    const sessionResult = await query(
