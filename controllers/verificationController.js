...
export const sendSMSOTP = async (req, res) => {
  try {
    ...
    // Store phone number in session for verification later
    await query(
      `UPDATE votteryy_auth_sessions SET phone_number = $1 WHERE session_id = $2`,
      [phone, sessionId]
    );

    // Send SMS via Twilio Verify (OTP is generated by Twilio)
    let smsSent = false;
    if (config.TWILIO.ACCOUNT_SID && config.TWILIO.AUTH_TOKEN && config.TWILIO.VERIFY_SERVICE_SID) {
      const result = await sendOTPSMS(phone);  // No OTP parameter needed!
